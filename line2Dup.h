...
public:
    ...
    public:
        ...
        Info(float angle_, float scale_, float c_x=0.0, float c_y=0.0){
            angle = angle_;
            scale = scale_;
            center_x = c_x;
            center_y = c_y;
    ...
    static void save_infos(std::vector<shapeInfo_producer::Info>& infos, std::string path = "infos.yaml"){
        ...
        for (int i = 0; i < infos.size(); i++)
        {
            ...
            fs << "center_x" << infos[i].center_x;
            fs << "center_y" << infos[i].center_y;
            fs << "}";
        }
        fs << "]";
    }
    static std::vector<Info> load_infos(std::string path = "info.yaml"){
        ...
        {
            infos.emplace_back(float((*it)["angle"]), float((*it)["scale"]), float((*it)["center_x"]), float((*it)["center_y"]));
        }
